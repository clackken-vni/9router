...
function deriveInternalMethod(url, body) {
  // From body.method (JSON-RPC style)
  if (body?.method && typeof body.method === "string") {
    return body.method;
  }
  // From query param (GET style)
...
export async function POST(request) {
  ...
  try {
    ...
    const requestInfo = buildRequestInfo(request, url, body, auth.token, {});
    ...
    // Proxy to upstream
    const response = await proxyToUpstream(request, url, body, auth.settings, {});
...
export async function GET(request) {
  ...
  try {
    ...
    const url = new URL(request.url);
    const requestInfo = buildRequestInfo(request, url, null, auth.token, {});
    ...
    // Proxy to upstream
    const response = await proxyToUpstream(request, url, null, auth.settings, {});
